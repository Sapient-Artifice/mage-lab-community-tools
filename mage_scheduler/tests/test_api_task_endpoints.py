...
Covers:
  ...
  - POST /api/tasks/{id}/cancel   (status guards, cascade, revoke, 404)
...
class TestCancelTask:
    ...
    def test_cancel_cascades_to_waiting_dependents(self, api_client):
        ...
        # Verify the dependent task is now failed
        s2 = _session(factory)
        try:
    ...
    def test_cancel_revokes_celery_task(self, api_client, monkeypatch):
        """When celery_task_id is set, AsyncResult.revoke must be called."""
        client, factory = api_client
        s = _session(factory)
        try:
            from models import TaskRequest
        ...
        monkeypatch.setattr(api_module, "AsyncResult", mock_async_result)

        resp = client.post(f"/api/tasks/{task_id}/cancel")
        assert resp.status_code == 200
        mock_async_result.assert_called_once_with("abc-123", app=api_module.celery_app)
...
class TestCreateTask:
    ...
    def test_create_task_notify_on_complete(self, api_client, monkeypatch):
        client, factory = api_client
        payload = {
            "command": "echo hello",
            "run_at": _run_at_str(),
